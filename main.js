...
class Player {
    ...
    check(){
        ...
        if(!dallFacingBack && this.playerInfo.velocity > 0){
            text.innerText = this.playerInfo.name + " perdiste!!!"
            this.playerInfo.isDead = true
            this.stop()
            DEAD_PLAYERS++
            loseMusic.play()
            if(DEAD_PLAYERS == players.length){
                text.innerText = "Perdieron todos!!!"
                gameStat = "ended"
            }
        ...
        if(this.playerInfo.positionX < end_position + .7){
            text.innerText = this.playerInfo.name + " estas seguro!!!"
            this.playerInfo.isDead = true
            this.stop()
            SAFE_PLAYERS++
            winMusic.play()
            if(SAFE_PLAYERS == players.length){
                text.innerText = "Todos están seguros!!!"
                gameStat = "ended"
            }
            if(DEAD_PLAYERS + SAFE_PLAYERS == players.length){
                gameStat = "ended"
...
async function init(){
    await delay(500)
    text.innerText = "Prevenido en 3"
    await delay(500)
    text.innerText = "Prevenido in 2"
    await delay(500)
    text.innerText = "Prevenido in 1"
    lookBackward()
    await delay(500)
    text.innerText = "Avanza!!!"
    bgMusic.play()
    start()
}
...
function start(){
    ...
    setTimeout(() => {
        if(gameStat != "ended"){
            text.innerText = "Se te acabó el tiempo!!!"
            loseMusic.play()
            gameStat = "ended"
        }
    }, TIME_LIMIT * 1000)
